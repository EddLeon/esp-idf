...
static void selfConfigure(void)
{
		...
		ESP_LOGE(TAG,"self-configuring...");

		//Add Application Key /
		bt_mesh_cfg_app_key_add(&ctx, net_idx, app_idx, app_key);

		ESP_LOGE(TAG,"app key configured");

		// Bind to generic onoff client model //
		err = bt_mesh_cfg_mod_app_bind(&ctx, node_addr, app_idx, ESP_BLE_MESH_MODEL_ID_GEN_ONOFF_CLI);
		if (err)
		{
			ESP_LOGE(TAG,"ERROR binding to generic onoff client model (err %d)\n", err);
				return;
		}
		else
		{
	ESP_LOGE(TAG,"bound appkey to generic onoff server model");
		}

		// Bind to Health model //
		bt_mesh_cfg_mod_app_bind(&ctx, node_addr, app_idx, ESP_BLE_MESH_MODEL_ID_HEALTH_SRV);

	ESP_LOGE(TAG,"self-configuration complete\n");
}
...
static int selfProvision(void) 
{
	...
	int err = bt_mesh_provision(net_key, net_idx, flags, iv_index, 0, node_addr, dev_key);
	if (err)
	{
		ESP_LOGE(TAG,"Provisioning failed (err %d)\n", err);
			return err;
	}
	ESP_LOGE(TAG,"Provisioning completed\n");
