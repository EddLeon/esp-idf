        ...
        esp_ble_mesh_model_t *model, uint32_t opcode)
{
    ...
    //debugg
    ESP_LOGI(TAG, "SET-MSG_COMMON unicast: 0x%04x",node->unicast);
    ESP_LOGI(TAG, "SET-MSG_COMMON prov_key.net_idx: 0x%04x",prov_key.net_idx);
    ESP_LOGI(TAG, "SET-MSG_COMMON prov_key.app_idx 0x%04x",prov_key.app_idx);//enddebugg
        ...
        esp_ble_mesh_generic_client_cb_param_t *param)
{
    ...
    //debugg
    ESP_LOGI(TAG, "%s TESTING UNICAST FROM NODE GET, addr: 0x%04x", __func__,node->unicast );
	//enddebugg	    
    if (!node) {
        ESP_LOGE(TAG, "%s: Get node info failed", __func__);
        return;
